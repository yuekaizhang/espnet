...
"""ContextNet speech recognition model (pytorch)."""
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def add_arguments(parser):
        """Add arguments."""
        group = parser.add_argument_group("contextnet model setting")
        group.add_argument(
            "--num-blocks",
            default=None,
        ...
        group.add_argument(
            "--residuals",
            default=None,
            type=lambda s: [bool(int(mod)) for mod in s.split("_") if s != ""],
            help=""
        )
        group.add_argument(
            "--activation",
        ...
        group.add_argument(
            ...
            help="how to initialize context parameters",
        )
        group.add_argument(
            "--contextnet-lr",
            default=10.0,
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        if args.mtlalpha < 1:
            raise ValueError("Currently only support CTC loss")
        ...
        if args.mtlalpha > 0.0:
            self.ctc = CTC(
                odim, args.filters[-1], args.dropout_rate, ctc_type=args.ctc_type, reduce=True
            )
        else:
            raise ValueError("Currently only support CTC loss")
            self.ctc = None
    ...
    def reset_parameters(self, args):
        ...
        # initialize parameters
        initialize(self, args.contextnet_init)
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        logging.info(f"The shape of xs_pad {xs_pad.shape}")
        hs_pad = self.encoder(xs_pad.transpose(1, 2))
        hs_pad = hs_pad.transpose(1, 2)
        logging.info(f"The shape of hs_pad {hs_pad.shape}")
        input()
        ...
        if self.mtlalpha == 0.0:
            ...
            # need to implement real mask
            hs_len = torch.ones([batch_size]) * hs_pad.size(1)
            loss_ctc = self.ctc(hs_pad, hs_len, ys_pad)
            if self.error_calculator is not None:
                ys_hat = self.ctc.argmax(hs_pad).data
